...
class EnhancedTreeVisualizer {
    ...
    drawBackground() {
        ...
        const gradient = this.ctx.createRadialGradient(centerX + Math.sin(this.time) * 100, centerY + Math.cos(this.time) * 100, 0, centerX, centerY, radius);
    ...
    drawNode(node) {
        ...
        // Main node gradient
        const nodeGradient = this.ctx.createRadialGradient(node.x - 15, node.y - 15, 0, node.x, node.y, finalRadius);
        ...
        this.ctx.fill();
        // Enhanced border
        const borderGradient = this.ctx.createLinearGradient(node.x - finalRadius, node.y - finalRadius, node.x + finalRadius, node.y + finalRadius);
